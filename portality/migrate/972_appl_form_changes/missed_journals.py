...
'''
Use a scroll search to remove the No CC-Like licence option in the DOAJ.
'''
...
license_correct_dict = {
                         "Not CC-like" : "Publisher's own license"
                        }
...
conn = raw.make_connection(app.config.get("ELASTIC_SEARCH_HOST"), app.config.get("ELASTIC_SEARCH_DB"))
...
for article_issn in missed_articles:
    ...
    for a in tasks.scroll(conn, 'article', query):
        try:
            ...
            print "Failed to create a model, id {}".format(a.get('id', ''))
        except KeyError:
            print "No license to change, id {}".format(a.get('id', ''))
