...
def fqw_hit():
    ...
    if page is not None:
        plausible.send_event(app.config.get('PLAUSIBLE_CATEGORY_FQW', 'FQW'),
                             action=app.config.get('PLAUSIBLE_ACTION_FQW', 'hit'),
...
@blueprint.route("/csv")
@plausible.pa_event(app.config.get('PLAUSIBLE_CATEGORY_JOURNALCSV', 'JournalCSV'),
                    action=app.config.get('PLAUSIBLE_ACTION_JOURNALCSV', 'Download'))
def csv_data():
