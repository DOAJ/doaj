'''
This is an in-development endpoint for passing through etherpad requests.
It will probably be totally changed to use etherpad-lite session auth stuff.
...
blueprint = Blueprint('padthru', __name__)
...
def padthru(path=''):
    addr = app.config['JSITE_OPTIONS']['pads_address']
    if path: addr += '/' + path
    print addr, dict(request.args), request.headers
    if request.method == 'GET':
        r = requests.get(addr, params=request.args, headers=request.headers, stream=True)
    #elif request.method == 'POST':
    #    r = requests.post(addr, data=request.data, params=request.args, headers=request.headers, allow_redirects=True)
    fr = Response(
        response=r.raw.read(),
