...
DOAJ_PRILIST_KEY_PATH
    `DOAJ_PRILIST_KEY_PATH` is path of json file path for google drive api, which generated by 
    `console.cloud.google.com` and should be enabled for
        * google drive api
        * google sheet api
        
        
DOAJ_GITHUB_KEY
    `DOAJ_GITHUB_KEY` is github api key, this key is optional, if not provided, you can input github 
    password instead by -p option
    

Example
---------------------
github_prioritisation -g 'test-githubpri' -r '/tmp/githubpri-rule.csv'
