...
def update_request(q):
    ...
    q.add_must_filter({"term" : {"admin.application_type" : constants.APPLICATION_TYPE_UPDATE_REQUEST}})
    return q


def not_update_request(q):
    q.clear_match_all()
    q.add_must_not({"term" : {"admin.application_type" : constants.APPLICATION_TYPE_UPDATE_REQUEST}})
    return q
