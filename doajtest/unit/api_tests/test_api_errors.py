...
class TestApiErrors(DoajTestCase):
    ...
    def test_01_api_404(self):
        with self.app_test.test_client() as t_client:
            ...
            response = t_client.get('/api/docs')
            assert response.status_code == 200
            assert response.mimetype == 'text/html'

            response = t_client.get('/api/not_valid')
    ...
    def test_02_api_400(self):
        ...
        with self.app_test.test_client() as t_client:
            ...
            response = t_client.get('/api/search/journals/query_string?page=1')
            assert response.status_code == 200

            # a bad request, the page number is invalid
            response = t_client.get('/api/search/journals/query_string?page=@')
    ...
    def test_03_api_401(self):
        ...
        with self.app_test.test_client() as t_client:
            ...
            response = t_client.get('/api/search/applications/issn%3A0000-0000?api_key=' + a1_key)
            ...
            response = t_client.get('/api/search/applications/issn%3A0000-0000?api_key=' + spurious_key)
