...
class TestApiErrors(DoajTestCase):
    ...
    def test_03_api_401(self):
        ...
        with self.app_test.test_client() as t_client:
            ...
            assert b"Test Suggestion Title" in response.data
            ...
            response = t_client.get('/api/v1/search/applications/issn%3A0000-0000?api_key=' + a1_key)
            assert response.status_code == 200
            assert b"Test Suggestion Title" not in response.data
            ...
            assert response.mimetype == 'application/json'
            assert b"An API Key is required to access this." in response.data
