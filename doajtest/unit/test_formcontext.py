...
class TestFormContext(DoajTestCase):
    ...
    def test_11_publisher_reapplication(self):
        ...
        fc = formcontext.ApplicationFormFactory.get_form_context(role="publisher", form_data=MultiDict(REAPPLICATION_FORM) ,source=models.Suggestion(**REAPPLICATION_SOURCE))
        assert isinstance(fc, formcontext.PublisherReApplication)
        assert fc.form is not None
        assert fc.source is not None
        assert fc.form_data is not None
